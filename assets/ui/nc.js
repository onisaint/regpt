/*! ncUIkit 3.0.0-beta.34 | http://www.getuikit.com | (c) 2014 - 2017 YOOtheme | MIT License */

(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
	typeof define === 'function' && define.amd ? define('uikit', factory) :
	(global.ncUIkit = factory());
...
function transition(element, props, duration, transition) {
    ...
    return Promise.all(toNodes(element).map(function (element) { return new Promise(function (resolve, reject) {
            ...
            once(element, (transitionend + " " + transitioncancel), function (ref) {
                ...
                clearTimeout(timer);
                removeClass(element, 'nc-transition');
            ...
            addClass(element, 'nc-transition');
...
var Transition = {
    ...
    inProgress: function inProgress(element) {
        return hasClass(element, 'nc-transition');
    }

};

var animationcancel = 'animationcancel';
var animationPrefix = 'nc-animation-';
var clsCancelAnimation = 'nc-cancel-animation';
...
var boot = function (ncUIkit) {

    var connect = ncUIkit.connect;
    var disconnect = ncUIkit.disconnect;
    ...
    function init() {
        ...
        (new Observer(function (mutations) { return mutations.forEach(function (ref) {
                ...
                ncUIkit.update(createEvent('update', true, false, {mutation: true}), target, true);
        ...
        ncUIkit._initialized = true;
    }

    function apply(node, fn) {

        if (node.nodeType !== 1 || hasAttr(node, 'nc-no-boot')) {
            return;
        }
...
var globalAPI = function (ncUIkit) {

    var DATA = ncUIkit.data;

    ncUIkit.use = function (plugin) {
    ...
    ncUIkit.mixin = function (mixin, component) {
        component = (isString(component) ? ncUIkit.components[component] : component) || this;
    ...
    ncUIkit.extend = function (options) {

        options = options || {};

        var Super = this, name = options.name || Super.options.name;
        var Sub = createClass(name || 'ncUIkitComponent');
    ...
    ncUIkit.update = function (e, element, parents) {
    ...
    var container;
    Object.defineProperty(ncUIkit, 'container', {
...
var hooksAPI = function (ncUIkit) {

    ncUIkit.prototype._callHook = function (hook) {
        var this$1 = this;
    ...
    ncUIkit.prototype._callReady = function () {
    ...
    ncUIkit.prototype._callConnected = function () {
        var this$1 = this;
        ...
        if (!includes(ncUIkit.elements, this.$options.el)) {
            ncUIkit.elements.push(this.$options.el);
        }

        ncUIkit.instances[this._uid] = this;
    ...
    ncUIkit.prototype._callDisconnected = function () {
        ...
        var index = ncUIkit.elements.indexOf(this.$options.el);

        if (~index) {
            ncUIkit.elements.splice(index, 1);
        }

        delete ncUIkit.instances[this._uid];
    ...
    ncUIkit.prototype._callUpdate = function (e) {
        var this$1 = this;
...
var stateAPI = function (ncUIkit) {

    var uid = 0;

    ncUIkit.prototype.props = {};

    ncUIkit.prototype._init = function (options) {

        options = options || {};
        options = this.$options = mergeOptions(this.constructor.options, options, this);

        this.$el = null;
        this.$name = ncUIkit.prefix + hyphenate(this.$options.name);
        this.$props = {};
    ...
    ncUIkit.prototype._initData = function () {
        var this$1 = this;
    ...
    ncUIkit.prototype._initMethods = function () {
        var this$1 = this;
    ...
    ncUIkit.prototype._initComputeds = function () {
        var this$1 = this;
    ...
    ncUIkit.prototype._initProps = function (props) {
        var this$1 = this;
    ...
    ncUIkit.prototype._initEvents = function () {
        var this$1 = this;
    ...
    ncUIkit.prototype._unbindEvents = function () {
        this._events.forEach(function (unbind) { return unbind(); });
        this._events = [];
    };

    ncUIkit.prototype._initObserver = function () {
        var this$1 = this;
...
var instanceAPI = function (ncUIkit) {

    var DATA = ncUIkit.data;

    ncUIkit.prototype.$mount = function (el) {
    ...
    ncUIkit.prototype.$emit = function (e) {
        this._callUpdate(e);
    };

    ncUIkit.prototype.$update = function (e, parents) {
        ncUIkit.update(e, this.$options.el, parents);
    };

    ncUIkit.prototype.$reset = function (data) {
        this._callDisconnected();
        this._initProps(data);
        this._callConnected();
    };

    ncUIkit.prototype.$destroy = function (removeEl) {
...
var componentAPI = function (ncUIkit) {

    var DATA = ncUIkit.data;

    ncUIkit.components = {};

    ncUIkit.component = function (id, options) {

        var name = camelize(id);

        if (isPlainObject(options)) {
            options.name = name;
            options = ncUIkit.extend(options);
        } else if (isUndefined(options)) {
            return ncUIkit.components[name]
        } else {
            options.options.name = name;
        }

        ncUIkit.components[name] = options;

        ncUIkit[name] = function (element, data) {
            var i = arguments.length, argsArray = Array(i);
            while ( i-- ) argsArray[i] = arguments[i];


            if (isPlainObject(element)) {
                return new ncUIkit.components[name]({data: element});
            }

            if (ncUIkit.components[name].options.functional) {
                return new ncUIkit.components[name]({data: [].concat( argsArray )});
            }

            return element && element.nodeType ? init(element) : $$(element).map(init)[0];

            function init(element) {
                return ncUIkit.getComponent(element, name) || new ncUIkit.components[name]({el: element, data: data || {}});
            }

        };

        if (ncUIkit._initialized && !options.options.functional) {
            fastdom.measure(function () { return ncUIkit[name](("[nc-" + id + "],[data-nc-" + id + "]")); });
        }

        return ncUIkit.components[name];
    };

    ncUIkit.getComponents = function (element) { return element && (element = isJQuery(element) ? element[0] : element) && element[DATA] || {}; };
    ncUIkit.getComponent = function (element, name) { return ncUIkit.getComponents(element)[name]; };

    ncUIkit.connect = function (node) {
        ...
        for (var i = 0; i < node.attributes.length; i++) {
            ...
            if (startsWith(name, 'nc-') || startsWith(name, 'data-nc-')) {

                name = camelize(name.replace('data-nc-', '').replace('nc-', ''));
    ...
    ncUIkit.disconnect = function (node) {
...
var ncUIkit$2 = function (options) {
    this._init(options);
};

ncUIkit$2.util = util;
ncUIkit$2.data = '__uikit__';
ncUIkit$2.prefix = 'nc-';
ncUIkit$2.options = {};
ncUIkit$2.instances = {};
ncUIkit$2.elements = [];
...
hooksAPI(ncUIkit$2);
stateAPI(ncUIkit$2);
instanceAPI(ncUIkit$2);
componentAPI(ncUIkit$2);
...
var Container = {
    ...
    computed: {
        ...
        container: function container(ref) {
            ...
            return container === true && ncUIkit$2.container || container && $(container) || ncUIkit$2.container;
        }
            ...
            return new Promise(function (resolve) {
                ...
                if (!this$1.queued || !isUndefined(animate) || !isUndefined(show) || !this$1.hasAnimation || targets.length < 2) {
                    ...
                    var body = doc.body,
                        ...
                        el = toggled[0],
                        inProgress = Animation.inProgress(el) && hasClass(el, 'nc-animation-leave')
        ...
        _toggleElement: function _toggleElement(el, show, animate) {
            ...
            show = isBoolean(show)
                ...
                : Animation.inProgress(el)
                    ? hasClass(el, 'nc-animation-leave')
...
var mixin = function (ncUIkit) {

    ncUIkit.mixin.class = Class;
    ncUIkit.mixin.container = Container;
    ncUIkit.mixin.modal = Modal;
    ncUIkit.mixin.position = Position;
    ncUIkit.mixin.togglable = Togglable;

};

var Accordion = function (ncUIkit) {

    ncUIkit.component('accordion', {
        ...
        defaults: {
            ...
            multiple: false,
            clsOpen: 'nc-open',
            toggle: '> .nc-accordion-title',
            content: '> .nc-accordion-content',
            transition: 'ease'
        },
...
var Alert = function (ncUIkit) {

    ncUIkit.component('alert', {
...
var Cover = function (ncUIkit) {

    ncUIkit.component('cover', {

        mixins: [Class, ncUIkit.components.video.options],
...
var Drop = function (ncUIkit) {

    var active;

    ncUIkit.component('drop', {
        ...
        defaults: {
            ...
            clsDrop: false,
            hoverIdle: 200,
            animation: ['nc-animation-fade'],
            cls: 'nc-open'
        },

        init: function init() {
            this.tracker = new MouseTracker();
            this.clsDrop = this.clsDrop || ("nc-" + (this.$options.name));
        ...
        ready: function ready() {
            ...
            if (this.toggle) {
                this.toggle = ncUIkit.toggle(isString(this.toggle) ? query(this.toggle, this.$el) : this.$el.previousElementSibling, {target: this.$el, mode: this.mode});
            }
    ...
    ncUIkit.drop.getActive = function () { return active; };
...
var Dropdown = function (ncUIkit) {

    ncUIkit.component('dropdown', ncUIkit.components.drop.extend({name: 'dropdown'}));

};

var FormCustom = function (ncUIkit) {

    ncUIkit.component('form-custom', {
...

            {
                ...
                handler: function handler(ref) {
                    ...
                    if (current === this.input) {
                        toggleClass(
                            this.state,
                            ("nc-" + (includes(type, 'focus') ? 'focus' : 'hover')),
...
var Gif = function (ncUIkit) {

    ncUIkit.component('gif', {
...
var Grid = function (ncUIkit) {

    ncUIkit.component('grid', ncUIkit.components.margin.extend({
        ...
        defaults: {
            margin: 'nc-grid-margin',
            clsStack: 'nc-grid-stack'
        },
...
var HeightMatch = function (ncUIkit) {

    ncUIkit.component('height-match', {
...
var HeightViewport = function (ncUIkit) {

    ncUIkit.component('height-viewport', {
...
var Hover = function (ncUIkit) {
    ...
    ready(function () {
        ...
        Object.defineProperty(ncUIkit, 'hoverSelector', {
        ...
        ncUIkit.hoverSelector = '.nc-animation-toggle, .nc-transition-toggle, [nc-hover]';
...
var Icon = function (ncUIkit) {
    ...
    ncUIkit.component('icon', ncUIkit.components.svg.extend({
    ...
    [
        ...
        init: function init() {
            addClass(this.$el, 'nc-slidenav');

            if (hasClass(this.$el, 'nc-slidenav-large')) {
                this.icon += '-large';
            }
        }
    ...
    registerComponent('search-icon', {
        ...
        init: function init() {
            if (hasClass(this.$el, 'nc-search-icon') && parents(this.$el, '.nc-search-large').length) {
                this.icon = 'search-large';
            } else if (parents(this.$el, '.nc-search-navbar').length) {
    ...
    registerComponent('close', {
        ...
        init: function init() {
            this.icon = "close-" + (hasClass(this.$el, 'nc-close-large') ? 'large' : 'icon');
        }
    ...
    ncUIkit.icon.add = function (added) {
        Object.keys(added).forEach(function (name) {
            icons[name] = added[name];
            delete parsed[name];
        });

        if (ncUIkit._initialized) {
            each(ncUIkit.instances, function (component) {
    ...
    function registerComponent(name, mixin$$1) {
        ...
        ncUIkit.component(name, ncUIkit.components.icon.extend({
...
var Leader = function (ncUIkit) {

    ncUIkit.component('leader', {
        ...
        defaults: {
            fill: '',
            media: false,
            clsWrapper: 'nc-leader-fill',
            clsHide: 'nc-leader-hide',
            attrFill: 'data-fill'
        },
...
var Margin = function (ncUIkit) {

    ncUIkit.component('margin', {
        ...
        defaults: {
            margin: 'nc-margin-small-top',
            firstColumn: 'nc-first-column'
        },
...
var Modal$1 = function (ncUIkit) {

    ncUIkit.component('modal', {

        mixins: [Modal],

        defaults: {
            clsPage: 'nc-modal-page',
            clsPanel: 'nc-modal-dialog',
            selClose: '.nc-modal-close, .nc-modal-close-default, .nc-modal-close-outside, .nc-modal-close-full'
        },
        ...
        events: [

            {
                ...
                handler: function handler() {
                    ...
                    if (hasClass(this.panel, 'nc-margin-auto-vertical')) {
                        addClass(this.$el, 'nc-flex');
                    } else {
    ...
    ncUIkit.component('overflow-auto', {
        ...
        computed: {
            ...
            modal: function modal(_, $el) {
                return closest($el, '.nc-modal');
            },

            panel: function panel(_, $el) {
                return closest($el, '.nc-modal-dialog');
            }
    ...
    ncUIkit.modal.dialog = function (content, options) {

        var dialog = ncUIkit.modal((" <div class=\"nc-modal\"> <div class=\"nc-modal-dialog\">" + content + "</div> </div> "), options);
    ...
    ncUIkit.modal.alert = function (message, options) {

        options = assign({bgClose: false, escClose: false, labels: ncUIkit.modal.labels}, options);

        return new Promise(
            function (resolve) { return on(ncUIkit.modal.dialog((" <div class=\"nc-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"nc-modal-footer nc-text-right\"> <button class=\"nc-button nc-button-primary nc-modal-close\" autofocus>" + (options.labels.ok) + "</button> </div> "), options).$el, 'hide', resolve); }
        );
    };

    ncUIkit.modal.confirm = function (message, options) {

        options = assign({bgClose: false, escClose: false, labels: ncUIkit.modal.labels}, options);

        return new Promise(
            function (resolve, reject) { return on(ncUIkit.modal.dialog((" <div class=\"nc-modal-body\">" + (isString(message) ? message : html(message)) + "</div> <div class=\"nc-modal-footer nc-text-right\"> <button class=\"nc-button nc-button-default nc-modal-close\">" + (options.labels.cancel) + "</button> <button class=\"nc-button nc-button-primary nc-modal-close\" autofocus>" + (options.labels.ok) + "</button> </div> "), options).$el, 'click', '.nc-modal-footer button', function (ref) {
                var target = ref.target;
    ...
    ncUIkit.modal.prompt = function (message, value, options) {
        ...
        return new Promise(function (resolve) {
            ...
            var resolved = false,
                prompt = ncUIkit.modal.dialog((" <form class=\"nc-form-stacked\"> <div class=\"nc-modal-body\"> <label>" + (isString(message) ? message : html(message)) + "</label> <input class=\"nc-input\" autofocus> </div> <div class=\"nc-modal-footer nc-text-right\"> <button class=\"nc-button nc-button-default nc-modal-close\" type=\"button\">" + (options.labels.cancel) + "</button> <button class=\"nc-button nc-button-primary\">" + (options.labels.ok) + "</button> </div> </form> "), options),
...
var Nav = function (ncUIkit) {

    ncUIkit.component('nav', ncUIkit.components.accordion.extend({
...
var Navbar = function (ncUIkit) {

    ncUIkit.component('navbar', {
        ...
        defaults: {
            dropdown: '.nc-navbar-nav > li',
            align: !isRtl ? 'left' : 'right',
            clsDrop: 'nc-navbar-dropdown',
            mode: undefined,
            offset: undefined,
            delayShow: undefined,
            delayHide: undefined,
        ...
        update: function update() {
            ...
            ncUIkit.drop(
                $$(((this.dropdown) + " ." + (this.clsDrop)), this.$el).filter(function (el) { return !ncUIkit.getComponent(el, 'dropdown'); }),
        ...
        methods: {
            ...
            getActive: function getActive() {
                var active = ncUIkit.drop.getActive();
    ...
    ncUIkit.component('navbar-dropbar', {
        ...
        init: function init() {
            ...
            if (this.mode === 'slide') {
                addClass(this.$el, 'nc-navbar-dropbar-slide');
            }
...
var Offcanvas = function (ncUIkit) {

    ncUIkit.component('offcanvas', {
        ...
        defaults: {
            content: '.nc-offcanvas-content',
            mode: 'slide',
            flip: false,
            overlay: false,
            clsPage: 'nc-offcanvas-page',
            clsContainer: 'nc-offcanvas-container',
            clsPanel: 'nc-offcanvas-bar',
            clsFlip: 'nc-offcanvas-flip',
            clsContent: 'nc-offcanvas-content',
            clsContentAnimation: 'nc-offcanvas-content-animation',
            clsSidebarAnimation: 'nc-offcanvas-bar-animation',
            clsMode: 'nc-offcanvas',
            clsOverlay: 'nc-offcanvas-overlay',
            selClose: '.nc-offcanvas-close'
        },
...
var Responsive = function (ncUIkit) {

    ncUIkit.component('responsive', {

        props: ['width', 'height'],

        init: function init() {
            addClass(this.$el, 'nc-responsive-width');
        },
...
var Scroll = function (ncUIkit) {

    ncUIkit.component('scroll', {
...
var Scrollspy = function (ncUIkit) {

    ncUIkit.component('scrollspy', {
        ...
        defaults: {
            ...
            repeat: false,
            delay: 0,
            inViewClass: 'nc-scrollspy-inview'
        },
...

            {
                ...
                read: function read() {
                    ...
                    this.elements.forEach(function (el) {
                        ...
                        if (!el._scrollspy) {
                            var cls = attr(el, 'nc-scrollspy-class');
...
var ScrollspyNav = function (ncUIkit) {

    ncUIkit.component('scrollspy-nav', {
        ...
        update: [

            {
                ...
                read: function read() {
                    if (this.scroll) {
                        ncUIkit.scroll(this.links, {offset: this.offset || 0});
                    }
                }
...
var Sticky = function (ncUIkit) {

    ncUIkit.component('sticky', {
        ...
        defaults: {
            ...
            clsActive: 'nc-active',
            clsInactive: '',
            clsFixed: 'nc-sticky-fixed',
            clsBelow: 'nc-sticky-below',
            selTarget: '',
            widthElement: false,
            showOnUp: false,
            media: false,
        ...
        connected: function connected() {
            ...
            this.placeholder = $$1('<div class="nc-sticky-placeholder"></div>');
...
var Svg = function (ncUIkit) {

    ncUIkit.component('svg', {
...
var Switcher = function (ncUIkit) {

    ncUIkit.component('switcher', {
        ...
        defaults: {
            ...
            swiping: true,
            cls: 'nc-active',
            clsContainer: 'nc-switcher',
            attrItem: 'nc-switcher-item',
            queued: true
        },
        ...
        events: [

            {
                ...
                delegate: function delegate() {
                    return ((this.toggle) + ":not(.nc-disabled)");
                },
        ...
        methods: {
            ...
            show: function show(item) {
                ...
                for (var i = 0; i < length; i++, next = (next + dir + length) % length) {
                    if (!matches(this$1.toggles[next], '.nc-disabled, [disabled]')) {
...
var Tab = function (ncUIkit) {

    ncUIkit.component('tab', ncUIkit.components.switcher.extend({
        ...
        defaults: {
            media: 960,
            attrItem: 'nc-tab-item'
        },

        init: function init() {

            var cls = hasClass(this.$el, 'nc-tab-left')
                ? 'nc-tab-left'
                : hasClass(this.$el, 'nc-tab-right')
                    ? 'nc-tab-right'
                    : false;

            if (cls) {
                ncUIkit.toggle(this.$el, {cls: cls, mode: 'media', media: this.media});
            }
        }
...
var Toggle = function (ncUIkit) {

    ncUIkit.component('toggle', {

        mixins: [ncUIkit.mixin.togglable],
...
var Video = function (ncUIkit) {

    ncUIkit.component('video', {
...
var core = function (ncUIkit) {

    var scroll = 0, started = 0;

    on(win, 'load resize', ncUIkit.update);
    ...
    animationstart && on(doc, animationstart, function (ref) {
        ...
        if ((css(target, 'animationName') || '').match(/^nc-.*(left|right)/)) {
            started++;
    ...
    // core components
    ncUIkit.use(Toggle);
    ncUIkit.use(Accordion);
    ncUIkit.use(Alert);
    ncUIkit.use(Video);
    ncUIkit.use(Cover);
    ncUIkit.use(Drop);
    ncUIkit.use(Dropdown);
    ncUIkit.use(FormCustom);
    ncUIkit.use(HeightMatch);
    ncUIkit.use(HeightViewport);
    ncUIkit.use(Hover);
    ncUIkit.use(Margin);
    ncUIkit.use(Gif);
    ncUIkit.use(Grid);
    ...
    ncUIkit.use(Nav);
    ncUIkit.use(Navbar);
    ncUIkit.use(Offcanvas);
    ncUIkit.use(Responsive);
    ncUIkit.use(Scroll);
    ncUIkit.use(Scrollspy);
    ncUIkit.use(ScrollspyNav);
    ncUIkit.use(Sticky);
    ncUIkit.use(Svg);
    ncUIkit.use(Icon);
    ncUIkit.use(Switcher);
    ncUIkit.use(Tab);

};

ncUIkit$2.version = '3.0.0-beta.34';

mixin(ncUIkit$2);
core(ncUIkit$2);

function plugin(ncUIkit) {

    if (plugin.installed) {
        return;
    }

    var ref = ncUIkit.util;
    var $ = ref.$;
    var doc = ref.doc;
    var empty = ref.empty;
    var html = ref.html;

    ncUIkit.component('countdown', {

        mixins: [ncUIkit.mixin.class],
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin);
}

function plugin$1(ncUIkit) {
    ...
    ncUIkit.component('grid-parallax', ncUIkit.components.grid.extend({
    ...
    ncUIkit.component('grid-parallax').options.update.unshift({
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$1);
}

var Animations = function (ncUIkit) {
...
function plugin$3(ncUIkit) {
    ...
    ncUIkit.mixin.slideshow = {
        ...
        defaults: {
            ...
            percent: 0,
            clsActive: 'nc-active',
            clsActivated: 'nc-transition-active',
            initialAnimation: false,
            Animations: Animations(ncUIkit)
        },
        ...
        methods: {
            ...
            move: function move(e) {
                ...
                ncUIkit.update(null, current);
                ncUIkit.update(null, next);
            },
            ...
            show: function show(index, force) {
                ...
                var prevIndex = this.index,
                    ...
                    prev = hasClass(this.slides, 'nc-active') && this.slides[prevIndex],
                    next = this.slides[nextIndex];
...
var Animations$1 = function (ncUIkit) {

    var mixin = ncUIkit.mixin;
    var ref = ncUIkit.util;
    var assign = ref.assign;
    var css = ref.css;
...
function plugin$2(ncUIkit) {

    if (plugin$2.installed) {
        return;
    }

    ncUIkit.use(plugin$3);

    var mixin = ncUIkit.mixin;
    var util = ncUIkit.util;
    var $ = util.$;
    var $$ = util.$$;
    ...
    ncUIkit.component('lightbox', {
        ...
        methods: {
            ...
            _init: function _init() {
                return this.panel = this.panel || ncUIkit.lightboxPanel(assign({}, this.$props, {
    ...
    ncUIkit.component('lightbox-panel', {
        ...
        created: function created() {
            ...
            this.caption = $('.nc-lightbox-caption', this.$el);
        ...
        events: [
...

            {
                ...
                handler: function handler(_, item) {
                    ...
                    this.setItem(item, '<span nc-spinner></span>');
                    ...
                    // Video
                        ...
                        var video = $(("<video controls playsinline" + (item.poster ? (" poster=\"" + (item.poster) + "\"") : '') + " nc-video=\"autoplay: " + (this.videoAutoplay) + "\"></video>"));
                    ...
                    // Iframe
                        ...
                        this.setItem(item, ("<iframe class=\"nc-lightbox-iframe\" src=\"" + source + "\" frameborder=\"0\" allowfullscreen></iframe>"));
        ...
        methods: {
            ...
            setItem: function setItem(item, content) {
                ...
                ncUIkit.update(null, el);
            },

            setError: function setError(item) {
                this.setItem(item, '<span nc-icon="icon: bolt; ratio: 2"></span>');
            },
            ...
            showControls: function showControls() {
                ...
                addClass(this.$el, 'nc-active nc-transition-active');

            },

            hideControls: function hideControls() {
                removeClass(this.$el, 'nc-active nc-transition-active');
            }
    ...
    function getIframe(src, width, height, autoplay) {
        return ("<iframe src=\"" + src + "\" width=\"" + width + "\" height=\"" + height + "\" style=\"max-width: 100%; box-sizing: border-box;\" frameborder=\"0\" allowfullscreen nc-video=\"autoplay: " + autoplay + "\" nc-responsive></iframe>");
    }

}

if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$2);
}

function plugin$4(ncUIkit) {
    ...
    ncUIkit.component('notification', {
        ...
        defaults: {
            ...
            group: null,
            pos: 'top-center',
            clsClose: 'nc-notification-close',
            clsMsg: 'nc-notification-message'
        },

        created: function created() {

            if (!containers[this.pos]) {
                containers[this.pos] = append(ncUIkit.container, ("<div class=\"nc-notification nc-notification-" + (this.pos) + "\"></div>"));
            }

            var container = css(containers[this.pos], 'display', 'block');

            this.$mount(append(container,
                ("<div class=\"" + (this.clsMsg) + (this.status ? (" " + (this.clsMsg) + "-" + (this.status)) : '') + "\"> <a href=\"#\" class=\"" + (this.clsClose) + "\" data-nc-close></a> <div>" + (this.message) + "</div> </div>")
            ));
    ...
    ncUIkit.notification.closeAll = function (group, immediate) {
        each(ncUIkit.instances, function (component) {
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$4);
}

function plugin$5(ncUIkit) {
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$5);
}

var Animations$2 = function (ncUIkit) {
...
function plugin$6(ncUIkit) {

    if (plugin$6.installed) {
        return;
    }

    ncUIkit.use(plugin$5);
    ncUIkit.use(plugin$3);
    ...
    ncUIkit.component('slideshow', {
        ...
        defaults: {
            ...
            minHeight: false,
            maxHeight: false,
            selList: '.nc-slideshow-items',
            attrItem: 'nc-slideshow-item',
            Animations: Animations$2(ncUIkit)
        },
    ...
    ncUIkit.component('slideshow-parallax', {
        ...
        computed: {
            ...
            item: function item() {
                var slideshow = ncUIkit.getComponent(closest(this.$el, '.nc-slideshow'), 'slideshow');
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$6);
}

function plugin$7(ncUIkit) {
    ...
    ncUIkit.component('sortable', {
        ...
        defaults: {
            ...
            animation: 150,
            threshold: 5,
            clsItem: 'nc-sortable-item',
            clsPlaceholder: 'nc-sortable-placeholder',
            clsDrag: 'nc-sortable-drag',
            clsDragState: 'nc-drag',
            clsBase: 'nc-sortable',
            clsNoDrag: 'nc-sortable-nodrag',
            clsEmpty: 'nc-sortable-empty',
            clsCustom: '',
            handle: false
        },
        ...
        methods: {
            ...
            start: function start(e) {
                ...
                this.drag = append(ncUIkit.container, this.placeholder.outerHTML.replace(/^<li/i, '<div').replace(/li>$/i, 'div>'));
    ...
    function getSortable(element) {
        return element && (ncUIkit.getComponent(element, 'sortable') || getSortable(element.parentNode));
    }
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$7);
}

function plugin$8(ncUIkit) {
    ...
    ncUIkit.component('tooltip', {
        ...
        defaults: {
            ...
            title: '',
            delay: 0,
            animation: ['nc-animation-scale-up'],
            duration: 100,
            cls: 'nc-active',
            clsPos: 'nc-tooltip'
        },
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$8);
}

function plugin$9(ncUIkit) {
...
if (!true && typeof window !== 'undefined' && window.ncUIkit) {
    window.ncUIkit.use(plugin$9);
}

ncUIkit$2.use(plugin);
ncUIkit$2.use(plugin$1);
ncUIkit$2.use(plugin$2);
ncUIkit$2.use(plugin$4);
ncUIkit$2.use(plugin$5);
ncUIkit$2.use(plugin$6);
ncUIkit$2.use(plugin$7);
ncUIkit$2.use(plugin$8);
ncUIkit$2.use(plugin$9);
